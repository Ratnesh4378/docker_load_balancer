...
import threading
import time
from multiprocessing import Manager, Process, Condition
app = Flask(__name__)

# Initialize replicas array
manager = Manager()
shared_data = manager.dict({'replicas': [], 'replica_requests': {}})
...
def get_replicas():
    ...
    with shared_condition:
        ...
        # Sort the replicas by their names
        sorted_replicas.sort()
        replica_requests = shared_data['replica_requests']
        ...
        with cond:
            ...
            for replica_url in list(replica_requests.keys()):
                if replica_url not in [r[1] for r in sorted_replicas]:
                    del replica_requests[replica_url]
        shared_data['replicas'] = replicas
        shared_data['replica_requests'] = replica_requests
